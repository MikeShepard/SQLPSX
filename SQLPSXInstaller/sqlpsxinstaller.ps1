$env:SQLPSX = "C:\Users\u00\Projects\SQLPSX_2.3\Modules"
...
#heat dir "$($env:SQLPSX)\PBM\Database" -nologo -sfrag -suid -ag -srd -dir Database -out PBMDatabase.wxs -cg PBMDatabase -dr Database -var var.PBMDatabase
#heat dir "$($env:SQLPSX)\PBM\Reports" -nologo -sfrag -suid -ag -srd -dir Reports -out PBMReports.wxs -cg PBMReports -dr Reports -var var.PBMReports
...
candle.exe sqlpsxAll.wxs adolib.wxs Agent.wxs ISECreamBasic.wxs OracleClient.wxs OracleIse.wxs PBM.wxs Repl.wxs ShowMbrs.wxs SQLIse.wxs SQLMaint.wxs SQLParser.wxs SQLPSX.wxs SQLServer.wxs SSIS.wxs WPK.wxs
...
light.exe -ext WixUIExtension -out SQLPSX.msi sqlpsxAll.wixobj adolib.wixobj Agent.wixobj ISECreamBasic.wixobj OracleClient.wixobj OracleIse.wixobj PBM.wixobj Repl.wixobj ShowMbrs.wixobj SQLIse.wixobj SQLMaint.wixobj SQLParser.wixobj SQLPSX.wixobj SQLServer.wixobj SSIS.wixobj WPK.wixobj -b "$($env:SQLPSX)"
